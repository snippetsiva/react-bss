...
export default function({data, headers, filename}) {
  ...
  headers.forEach((heading, index) => {
    // If we choose to, we can add a prop 'xPos' to the headers config in parent component
    ...
    if (heading.hasOwnProperty('xPos')) {
      doc.text(heading.name, heading.xPos, pageMargin)
      ...
      doc.text(heading.name, index === 0 ? xPositionForCurrentHeader:(xPositionForCurrentHeader + padding), yPositionForHeaders)
  ...
  data.forEach((row, rIndex) => {
    ...
    * */
    ...
    headers.forEach((column, cIndex) => {
      // const longText = doc.splitTextToSize(String(row[column.keyName]), xPositions[cIndex] - xPositions[cIndex !== 0 && cIndex - 1] )
      const longText = doc.splitTextToSize(String(row[column.keyName]),column.width?column.width:xPositions[cIndex] - xPositions[cIndex !== 0 && cIndex - 1] )
